%% GRIDMAKE
%
%  Expands dimensional coordinates into ordered Cartesian product of grid points
%
%  Usage
%    X = gridmake(x)
%    X = gridmake(x1,x2,x3,...)
...
%  Output
%    If N is the dx2 matrix that indexes the size of the inputs GRIDMAKE 
%    returns a prod(N(:,1)) by sum(N(:,2)) matrix. The output can also be 
%    returned as either as d matrices or sum(N(:,2)) matrices. If any of 
%    the inputs are grids, they are expanded internally. Thus
%      X = gridmake({x1,x2,x3})
%      X = gridmake(x1,x2,x3)
...
%     3     5
% 
%  The function performs action similar to NDGRID, the main difference is in 
%  the increased flexibility in specifying the form of the inputs and outputs.
