...
%
%  More specifically, uses method of collocation to solve Bellman equation
%  of discrete-time dynamic optimization model with mixed discrete and
%  continuous state and mixed discrete and continuous action, both of
%  arbitrary dimension. Use of routine is fully documented in the pdf file
%  `DPSOLVE - A MATLAB Routine for Solving Discrete-Time Bellman Equations'
%  included in the directory CompEcon2015 accompanying the distribution of
%  this file.  A summary of its features is provided here:
%
%  Usage
%    [c,sr,vr,xr,resid,n] = dpsolve(model,basis,v,x)
%  Let
...
%    n  = number of basis functions per continuous state dimension (1.ds)
%    nc = number of basis functions and collocation nodes (prod(n))
...
%    basis  : ds-dimensional basis defined on continuous state space
...
%    n      : (1.ds) number of basis functions per continuous state dimension 
...
%      ds        : dimension of continuous state variable (1)
%      dx        : dimension of continuous action variable (1)
...
%    continuous action x at an arbitrary number ns continuous states and
...
%  Options
%    algorithm : collocation equation solution algorithm, either function
%                iteration `funcit' or Newton's method `newton', the default
...
%    maxitncp  : maximum iterations, nonlinear complementarity solver (50)
...
function [c,sr,vr,xr,resid,n] = dpsolve(model,basis,v,x)
...
Phi = funbase(basis);       % collocation matrix
