%% REMSOLVE  
%
%  Solves discrete time rational expectations model via collocation
%
...
c = Phi\x;                  % initial basis coefficients
...
for it=1:maxit                                   
  ...
  c = Phi\x;                                      % update basis coefficient
...
if nres>0
  ...
  sr = gridmake(srcoord);
  xr = funeval(Phi\x,basis,sr);                     % rough guess for responses at evaluation points
